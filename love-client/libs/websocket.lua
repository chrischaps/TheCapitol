...
function _M.new(host, port, path)
    ...
    -- Use blocking connect (with timeout) since non-blocking is unreliable on Windows
    m.socket:settimeout(5)  -- 5 second timeout for connection
    local res, err = m.socket:connect(host, port)
    print("[websocket] blocking connect result: " .. tostring(res) .. ", err: " .. tostring(err))

    if res or err == "already connected" then
        -- Connection successful, switch to non-blocking for normal operation
        m.socket:settimeout(0)
        m.status = STATUS.CONNECTING
        -- Send upgrade request immediately
        m.socket:send(
"GET "..path.." HTTP/1.1\r\n"..
"Host: "..host..":"..port.."\r\n"..
"Connection: Upgrade\r\n"..
...
"Sec-WebSocket-Key: "..seckey.."\r\n\r\n")
        print("[websocket] upgrade request sent")
        m._length = 2
    else
        print("[websocket] connection failed: " .. tostring(err))
...
function _M:update()
    ...
    if self.status==STATUS.TCPOPENING then
        -- This state is no longer used - connection happens in new()
        -- If we get here, something went wrong
        self:onerror("unexpected state")
        self.status = STATUS.CLOSED
    elseif self.status==STATUS.CONNECTING then
        local res, err = sock:receive("*l")
        if res then
            print("[websocket] HTTP response: " .. res)
            repeat res = sock:receive("*l") until res==""
            print("[websocket] Upgrade complete! WebSocket is open.")
            self:onopen()
            self.status = STATUS.OPEN
        elseif err and err ~= "timeout" then
            print("[websocket] HTTP receive error: " .. tostring(err))
        end
